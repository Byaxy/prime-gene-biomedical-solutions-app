...
const ExpenseActions: React.FC<ExpenseActionsProps> = ({ expense }) => {
  const [openDialog, setOpenDialog] = useState(false);
  const [mode, setMode] = useState<"view" | "delete">("view");
  const router = useRouter();

  const handleEdit = () => {
    router.push(`/accounting-and-finance/expenses/edit/${expense.expense.id}`);
  };
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent className="w-72 bg-white py-4 px-2" align="end">
          ...
          >
            <EditIcon className="mr-2 h-4 w-4" /> Edit Expense
          </DropdownMenuItem>
          <DropdownMenuItem
