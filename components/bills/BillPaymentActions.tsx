...
const BillPaymentActions: React.FC<BillPaymentActionsProps> = ({
  ...
  const handleEdit = () => {
    if (hasPaymentRecord && billTrackerData.billPaymentId) {
      router.push(`/vendors/pay-bills/edit/${billTrackerData.billPaymentId}`);
    } else {
      router.push(
        `/vendors/pay-bills/create?purchaseId=${billTrackerData.purchase.id}`
      );
      toast.error(
        "No direct payment record to edit. Creating new payment for this purchase."
      );
    }
  };
