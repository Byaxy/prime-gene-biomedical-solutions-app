...
const DeliveryForm = ({ mode, initialData }: DeliveryFormProps) => {
  ...
  const handleSaleSelection = (saleId: string) => {
    ...
    if (
      ...
      selectedSale.sale.deliveryAddress.addressName !== ""
    ) {
      // use delivery address from sale
      const deliveryAddress = selectedSale.sale.deliveryAddress;
      ...
      const customerAddress = selectedSale.customer.address;

      if (customerAddress?.country) {
        const countryStates =
          State.getStatesOfCountry(customerAddress.country) || [];
        setStates(countryStates);
      ...
      form.setValue("deliveryAddress.country", customerAddress?.country || "");
      form.setValue("deliveryAddress.state", customerAddress?.state || "");
      form.setValue("deliveryAddress.city", customerAddress?.city || "");
      form.setValue(
        "deliveryAddress.addressName",
        customerAddress?.addressName || ""
      );
      form.setValue("deliveryAddress.address", customerAddress?.address || "");
      form.setValue(
    ...
    if (selectedSale.products.length > 0) {
      selectedSale.products.forEach((product: SaleItem) => {
        ...
        const productStock = product.inventoryStock.map((stock) => ({
          inventoryStockId: stock.inventoryStockId,
          lotNumber: stock.lotNumber,
          quantityTaken: stock.quantityToTake,
        }));
  ...
  }, [initialData, form, stores, remove, append, defaultValues]);
  ...
  const handleRefreshDeliveryRefNumber = async () => {
    if (mode === "create") {
      try {
        ...
        console.error("Error refreshing delivery reff number:", error);
        toast.error("Failed to refresh delivery reff number");
      }
    }
  };
