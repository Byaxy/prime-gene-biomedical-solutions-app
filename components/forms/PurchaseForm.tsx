...
interface PurchaseFormProps {
  ...
  generatedPurchaseNumber?: string;
...
const PurchaseForm = ({
  ...
  vendors,
  purchaseOrders,
  generatedPurchaseNumber: initialGeneratedPurchaseNumber,
  purchases,
}: PurchaseFormProps) => {
  ...
  const { addPurchase, editPurchase, isCreatingPurchase, isEditingPurchase } =
  ...
  const defaultValues = useMemo(
    () => ({
      purchaseNumber: initialGeneratedPurchaseNumber || "",
      vendorInvoiceNumber: "",
    ...
    [initialGeneratedPurchaseNumber]
  );
  ...
  useEffect(() => {
    if (
      ...
      const savedData = window.localStorage.getItem("purchaseForm");
      if (savedData) {
        try {
          const parsedData: PurchaseFormValues = JSON.parse(savedData);

          parsedData.purchaseDate = parsedData.purchaseDate
          ...
          window.localStorage.removeItem("purchaseForm");
        }
      } else {
        form.reset(defaultValues);
      }
    }
  }, [mode, defaultValues, form, sourcePurchaseOrder]);
  ...
  }, [mode, fields.length, form, sourcePurchaseOrder]);
    ...
    const subscription = form.watch((value) => {
      const timeoutId = setTimeout(() => {
        const dataToSave = JSON.stringify({
          ...value,
          purchaseDate:
            value.purchaseDate instanceof Date
              ? value.purchaseDate.toISOString()
              : value.purchaseDate,
          purchaseNumber: "",
        ...
        window.localStorage.setItem("purchaseForm", dataToSave);
      }, 500);
  ...
  useEffect(() => {
    if (initialMount.current) {
      ...
      } else if (mode === "create" && initialGeneratedPurchaseNumber) {
        form.setValue("purchaseNumber", initialGeneratedPurchaseNumber);
      }
  ...
  useEffect(() => {
    if (
      mode === "create" &&
      initialGeneratedPurchaseNumber &&
      form.getValues("purchaseNumber") === ""
    ) {
      form.setValue("purchaseNumber", initialGeneratedPurchaseNumber);
    }
  }, [initialGeneratedPurchaseNumber, form, mode]);
  ...
  const handleRefreshPurchaseNumber = async () => {
    if (mode === "create") {
      try {
        ...
        const newPurchaseNumber = await generatePurchaseNumber();
        form.setValue("purchaseNumber", newPurchaseNumber);
      } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const loadingToastId = toast.loading(
        mode === "create" ? "Creating purchase..." : "Updating purchase..."
      );
  ...
  const isAnyMutationLoading = isCreatingPurchase || isEditingPurchase;
        ...
        >
          <div className="w-full flex flex-col md:flex-row gap-5">
            ...
            <CustomFormField
              ...
              disabled={!!sourcePurchaseOrder || isAnyMutationLoading}
            >
          ...
          <div className="flex flex-col md:flex-row gap-5">
            <div className="flex flex-1 flex-row gap-2 items-center">
              <CustomFormField
                ...
                placeholder={
                  isRefetchingNumber ? "Generating..." : "Enter purchase number"
                }
